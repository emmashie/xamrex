...
class AMReXLazyArray(BackendArray):
    ...
    def __init__(self, fplt: Path, level: int, field_name: str,
                 ...
                 group_prefix: str = "Cell", group_dimensionality: int | None = None):
        self.fplt = fplt
        self.level = level
        ...
        self.group_dimensionality = group_dimensionality or meta.dimensionality
        group_fields = meta.multifab_groups.get(group_prefix, {}).get("fields", meta.field_list)
        self.field_idx = group_fields.index(field_name)
    ...
    def _read_fab_data(self, fab_idx):
        ...
        with open(fab_file, 'rb') as f:
            ...
            if self.group_dimensionality == 3:
                ...
                # 2D multifabs are usually written with dk=1; collapse k explicitly.
                fab_data_3d = np.array(field_data, dtype=np.float64).reshape((dk, dj, di))
                fab_data = fab_data_3d[0, :, :]
...
class AMReXSingleLevelStore(AbstractDataStore):
    ...
    def __init__(self, plotfile_path: Path, level: int = 0, time_dimension_name: str = None,
                 dimension_names: dict = None, include_auxiliary_multifabs: bool = False,
                 auxiliary_multifabs: list[str] | None = None):
        ...
        self.selected_groups = self.meta.resolve_multifab_groups(
        ...
        # Get fab metadata for each requested group at this level.
        self.fab_meta_by_group = {}
        for group in self.selected_groups:
            group_info = self.meta.multifab_groups[group]
            try:
                self.fab_meta_by_group[group] = AMReXFabsMetaSingleLevel(
                    plotfile_path,
                    group_info["n_fields"],
                    self.meta.dimensionality,
                    level,
                    prefix=group,
                )
            except FileNotFoundError:
                # Auxiliary groups may be absent at some levels; keep loading other groups.
                if group == "Cell":
        ...
        for group in self.selected_groups:
            if group not in self.fab_meta_by_group:
                continue
            for field in self.meta.multifab_groups[group]["fields"]:
                output_name = field
                if output_name in used_names:
                    output_name = f"{group}__{field}"
                used_names.add(output_name)
                self.output_fields.append((output_name, field, group))
    ...
    def get_attrs(self):
        ...
        return {
            ...
            'multifab_groups_loaded': self.selected_groups,
        }
...
class AMReXMultiTimeStore(AbstractDataStore):
    ...
    def __init__(self, plotfile_paths: list, level: int = 0,
        ...
        self.include_auxiliary_multifabs = include_auxiliary_multifabs
        self.auxiliary_multifabs = auxiliary_multifabs or []
        ...
        for path in self.plotfile_paths:
            try:
                ...
                meta_store = AMReXSingleLevelStore(
                    path,
                    0,
                    time_dimension_name,
                    dimension_names,
                    include_auxiliary_multifabs=include_auxiliary_multifabs,
                    auxiliary_multifabs=auxiliary_multifabs,
                )  # Always use level 0 for metadata
            ...
            if store.meta.max_level >= self.level:
                try:
                    ...
                    test_store = AMReXSingleLevelStore(
                        ...
                        include_auxiliary_multifabs=self.include_auxiliary_multifabs,
                        auxiliary_multifabs=self.auxiliary_multifabs,
                    )
    ...
    def get_variables(self):
        ...
        for field_name, _, _ in self.level_template_store.output_fields:
...
class AMReXEntrypoint(BackendEntrypoint):
    ...
    def open_dataset(
        ...
        **kwargs
        ...
        include_auxiliary_multifabs : bool, default False
        ...
        **kwargs
        ...
        if len(plotfile_paths) == 1:
            store = AMReXSingleLevelStore(
                ...
                include_auxiliary_multifabs=include_auxiliary_multifabs,
                auxiliary_multifabs=list(auxiliary_multifabs) if auxiliary_multifabs else None,
            )
