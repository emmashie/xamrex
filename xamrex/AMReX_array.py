...
class AMReXDatasetMeta():
    ...
    def _infer_multifab_dimensionality(self, prefix):
        """Infer spatial dimensionality for a multifab group from its prefix."""
        if prefix == "Cell":
            return self.dimensionality
        if prefix.endswith("2d"):
            return 2
        return self.dimensionality

    def _parse_additional_multifab_groups(self, trailing_lines):
        """Parse additional multifab groups listed after Cell entries in Header.

        Returns
        -------
        dict
            Mapping from group prefix to metadata with fields, n_fields, levels, and dimensionality.
        """
        groups = {
            "Cell": {
                "prefix": "Cell",
                "fields": self.field_list.copy(),
                "n_fields": self.n_fields,
                "levels": [f"Level_{lev}/Cell" for lev in range(self.max_level + 1)],
                "dimensionality": self.dimensionality,
            }
        }

        clean = [line.strip() for line in trailing_lines if line.strip()]
        if not clean:
            return groups

        level_path_re = re.compile(r"^Level_(\d+)/(\S+)$")

        for idx, line in enumerate(clean):
            match = level_path_re.match(line)
            if match is None:
                continue
            ...
            k = idx - 1
            while k >= 0 and not clean[k].isdigit():
                k -= 1
            if k < 0:
                continue
            ...
            ncomp = int(clean[k])
            names_start = k + 1
            names_end = names_start + ncomp
            if names_end > len(clean):
                continue

            field_names = [clean[n].strip() for n in range(names_start, names_end)]
            if idx != names_end:
                continue
            ...
            j = idx
            while j < len(clean):
                m_j = level_path_re.match(clean[j])
                if m_j is None:
                    break
                if m_j.group(2) != prefix:
                    break
                level_entries.append(clean[j])
                j += 1
            ...
            groups[prefix] = {
                ...
                "dimensionality": self._infer_multifab_dimensionality(prefix),
            }

        return groups

    def resolve_multifab_groups(self, include_auxiliary_multifabs=False, auxiliary_multifabs=None):
        """Resolve requested multifab groups to load.

        Parameters
        ----------
        include_auxiliary_multifabs : bool, optional
            If True, load all groups discovered in Header in addition to Cell.
        auxiliary_multifabs : list[str], optional
            Explicit list of auxiliary group prefixes (e.g., ['rho2d', 'u2d']).

        Returns
        -------
        list
            Ordered list of group prefixes to load.
        """
        groups = ["Cell"]
        if include_auxiliary_multifabs:
            groups.extend([g for g in self.multifab_groups.keys() if g != "Cell"])

        if auxiliary_multifabs:
            for group in auxiliary_multifabs:
                if group in self.multifab_groups and group not in groups:
                    groups.append(group)
    ...
    def _parse_dataset_header(self):
        ...
        with open(self.fdataset_header) as header_file:
            ...
            # Parse any additional multifab blocks after the Cell header section.
            trailing_lines = header_file.readlines()
            self.multifab_groups = self._parse_additional_multifab_groups(trailing_lines)
...
class AMReXFabsMetaSingleLevel():
    ...
    def __init__(self, fplt, n_fields, dimensionality, level, prefix="Cell"):
        """
        ...
        self.level = level
        self.prefix = prefix
        self.fheader_file = Path(self.fplt, f'Level_{level}/{prefix}_H')
